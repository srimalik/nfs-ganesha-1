			...
			struct nfs_resop4 *resp)
{
	...
	glist_for_each_safe(glist, glistn,
		...
		if (memcmp(iter_lock->sle_state->stateid_other,
	...
	if (found_lock->sle_state->state_seqid !=
	    arg_DELEGRETURN4->deleg_stateid.seqid) {
		LogDebug(COMPONENT_NFS_V4_LOCK,
			 "Found delegation but seqids don't match, "
			 "arg deleg seqid: %u, found deleg seqid: %u",
			 arg_DELEGRETURN4->deleg_stateid.seqid,
			 found_lock->sle_state->state_seqid);
