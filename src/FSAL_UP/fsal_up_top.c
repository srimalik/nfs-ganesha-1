				...
				rpc_call_t *call)
{
	...
	case  NFS4ERR_BAD_STATEID:
		LogDebug(COMPONENT_NFS_CB,
			 "Client sent NFS4ERR_BAD_STATEID response, revoking delegation (%p)",
			 p_cargs->deleg_entry);
		resp_action = REVOKE_SYNC;
		break;
	default:
		/* some other NFS error, consider the recall failed */
		LogDebug(COMPONENT_NFS_CB,
			 "Client sent %d response, revoking delegation (%p)",
			 call->cbt.v_u.v4.res.status,
			 p_cargs->deleg_entry);
		resp_action = RECALL_SCHED;
		break;
